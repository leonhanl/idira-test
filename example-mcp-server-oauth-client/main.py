...
CREDENTIALS_PATH = CLIENT_DIR / "lhan_mcp_client-credentials.json"
CONNECT_PATH = CLIENT_DIR / "lhan_mcp_client-connect.mcp-example.json"
...
async def list_example_tools(config: CyberArkConfig, redirect_uri: str) -> dict[str, Any]:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "List Example MCP server tools through the CyberArk Identity Broker."
        )
    )
    parser.add_argument(
        "--redirect-uri",
...
def main() -> None:
    args = parse_args()
    config = load_cyberark_config(CREDENTIALS_PATH, CONNECT_PATH)
    result = asyncio.run(list_example_tools(config, args.redirect_uri))
